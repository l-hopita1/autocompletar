__version__ = "2.3.0"
__repo__ = "https://github.com/l-hopita1/autocompletar"
__author__ = "Leonardo Hansen"
...
function onFormSubmitHandler(e) {
    ...
    SpreadsheetApp.getActive().getFormUrl().match(/[-\w]{25,}/)[0]
    );

    const nombreFormulario = form.getTitle();
    const urlFormulario = form.getPublishedUrl();
    if (!e) {
        throw new Error(`onFormSubmitHandler | No se ingresó ningún dato del formulario`);
    }
    ...
    if (!equipo || !hojaUrl || !carpetaUrl) {
        throw new Error(`❌ onFormSubmitHandler | No se ingestaron bien los datos del formulario. equipo: ${equipo}; hojaUrl:${hojaUrl}; carpetaUrl:${carpetaUrl}`);
    }
    else if (!hojaUrl.includes("spreadsheets")) {
        throw new Error("❌ onFormSubmitHandler | El enlace ingresado NO es una hoja de Google Sheets");
    }
    ...
    if (!plantillaUrl) {
        throw new Error(`❌ onFormSubmitHandler | No existe una plantilla para el equipo "${equipo}".`);
    }
    ...
    if (!codigoEquipo) {
        throw new Error(`❌ onFormSubmitHandler | No existe un código para el equipo "${equipo}".`);
    }
    const hoy = new Date();
    ...
    llenarPresupuesto(plantillaUrl, hojaUrl, carpetaUrl, numeroPresupuesto, nombreArchivo, equipo, nombreFormulario, urlFormulario);
}


function llenarPresupuesto(plantillaUrl, hojaUrl, carpetaUrl, numeroPresupuesto, nombreArchivo, equipo, nombreFormulario, urlFormulario) {
    ...
    reemplazarTextoEnDocumento(body, header, footer, marcador, valor, errores);
    ...
    enviarReporteErroresSiCorresponde(
        errores,
        nombreArchivo,
        numeroPresupuesto,
        libro.getName(),
 ...
 * @param {string} valor 
 */
function reemplazarTextoEnDocumento(body, header, footer, marcador, valor, errores) {
    try {
        ...
        Logger.log(`❌ reemplazarTextoEnDocumento | ${e.toString()}`);
        errores.texto.push(e.message);
    }
}
...
function insertarTablas(libro, body, datos, errores) {
    for (let i = 1; i < datos.length; i++) {
        try {
            ...
            if (!marcadorTabla && !nombreHoja && !celdaInicial && !celdaFinal) {
                ...
                throw new Error(`En tabla ${i}, falta el dato marcadorTabla: "${marcadorTabla}"`);
            } else if (!nombreHoja) {
                throw new Error(`En tabla ${i}, falta el dato nombreHoja: "${nombreHoja}"`);
            } else if (!celdaInicial) {
                throw new Error(`En tabla ${i}, falta el dato celdaInicial: "${celdaInicial}"`);
            } else if (!celdaFinal) {
                throw new Error(`En tabla ${i}, falta el dato celdaFinal: "${celdaFinal}"`);
            } else {
                const hojaTabla = libro.getSheetByName(nombreHoja);
                if (!hojaTabla) {
                    throw new Error(`No se encontró la hoja llamada "${nombreHoja}".`);
                }
                try {
                    hojaTabla.getRange(celdaInicial);
                    hojaTabla.getRange(celdaFinal);
                } catch (e) {
                    throw new Error(`Rango inválido: ${celdaInicial}:${celdaFinal}`);
                }
                ...
                if (!parrafoTabla) {
                    throw new Error(`No se encontró el marcador "${marcadorTabla}"`);
                }
                else {
            ...
            Logger.log(`❌ insertarTablas | ${e.toString()}`);
            errores.tablas.push(e.message);
        }
    }
}
 ...
 */
function insertarGraficosPorTitulo(libro, body, datos, errores) {
    try {
        for (let i = 1; i < datos.length; i++) {
            ...
            if (!marcadorGrafico || !nombreHoja || !tituloGrafico) {
                continue;
            } else if (!marcadorGrafico) {
                throw new Error(`En gráfico ${i}, no se encontró marcadorGrafico: ${marcadorGrafico}`);
            } else if (!marcadorGrafico) {
                throw new Error(`En gráfico ${i}, no se encontró nombreHoja: ${nombreHoja}`);
            } else if (!marcadorGrafico) {
                throw new Error(`En gráfico ${i}, no se encontró tituloGrafico: ${tituloGrafico}`);
            }
            else {
                ...
                if (!grafico) {
                    throw new Error(`No se encontró el gráfico con el título "${tituloGrafico}"`);
                }
                else {
                    ...
                    if (!textoEncontrado) {
                        throw new Error(`No se encontró el marcador "${marcadorGrafico}"`);
                    }
                    else {
        ...
        Logger.log(`❌ insertarGraficosPorTitulo | ${e.toString()}`);
        errores.graficos.push(e.message);
    }
}

function enviarReporteErroresSiCorresponde(errores, nombreArchivo, numeroPresupuesto, libro, carpeta, equipo, plantillaUrl, hojaUrl, carpetaUrl, nombreFormulario, urlFormulario) {

    const hayErrores =
        errores.texto.length ||
        errores.tablas.length ||
        errores.graficos.length;

    if (!hayErrores) {
        Logger.log("✅ enviarReporteErroresSiCorresponde | No hay errores → no se envía email");
        return;
    }

    let cuerpo = `
Se generó el presupuesto "${nombreArchivo}" con errores.

Datos del formulario:
- 📝 Formulario: ${nombreFormulario} → [${urlFormulario}]
- 🚀 Equipo: ${equipo} → [${plantillaUrl}]
- 📘 Libro: ${libro} → [${hojaUrl}]
- 📁 Carpeta: ${carpeta} → [${carpetaUrl}]

Errores en marcadores:
`;

    if (errores.texto.length) {
        cuerpo += `🔠 Texto:\n`;
        errores.texto.forEach(m =>
            cuerpo += ` - ${m}\n`);
        cuerpo += `\n`;
    }

    if (errores.tablas.length) {
        cuerpo += `📆 Tabla:\n`;
        errores.tablas.forEach(m =>
            cuerpo += ` - ${m}\n`
        );
        cuerpo += `\n`;
    }

    if (errores.graficos.length) {
        cuerpo += `📊 Gráfico:\n`;
        errores.graficos.forEach(m =>
            cuerpo += ` - ${m}\n`
        );
    }

    cuerpo+=`\n🤓 Datos nerd:\n`
    cuerpo+=`- Versión: ${__version__}\n`
    cuerpo+=`- Link de repositorio: ${__repo__}\n`
    cuerpo+=`- Desarrollador: ${__author__}\n`

    MailApp.sendEmail({
        to: Session.getActiveUser().getEmail(), // o fijo
        subject: `⚠️ Presupuesto ${numeroPresupuesto} generado con errores`,
        body: cuerpo
    });

    Logger.log("📤 enviarReporteErroresSiCorresponde | Reporte de errores enviado por email");
}
